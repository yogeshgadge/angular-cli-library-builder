...
function runNgc(project, tag) {
    return new Promise((resolve, reject) => {
        console.log(`${tag}:: Compiling...`);
        const exitCode = ngc(['--project', project ], (errorsAndWarningsString) => {
            if (errorsAndWarningsString) {
                if (errorsAndWarningsString.match(/: error TS([0-9]+):/)) {
                    console.log(`${tag}:: Compilation failed :(`);
                    reject(errorsAndWarningsString);
                } else {
                    console.log(`${tag}:: Compilation successful with warnings.`);
                    console.log(`${tag}:: Warnings\n${errorsAndWarningsString}`);
                    resolve(errorsAndWarningsString);
                }

            } else {
                console.log(`${tag}:: Compilation successful!`);
                resolve(true);
            }
        });
        if (exitCode === 0) {
            ...
            console.log(`${tag}:: Compilation failed :(`);
            reject(` Compilation failed with exit code  ${exitCode}`);

        }
    })
}

function runNgcBAK(project) {
    return new Promise((resolve, reject) => {
...
module.exports.build = function(options) {
    ...
    // Copy library to temporary folder and inline html/css.
        ...
        // Compile to ES2015.
        .then(() => runNgc(`${options.tempLibFolder}/tsconfig.lib.json` , 'ES2015 compilation'))
        // Compile to ES5.
        .then(() => runNgc(`${options.tempLibFolder}/tsconfig.es5.json` , 'ES5 compilation'))
